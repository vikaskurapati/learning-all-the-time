...
using namespace sycl;
  ...
  template<typename T, typename AtomicRef>
  void atomicUpdate(AtomicRef& atomic, T value){

    constexpr auto MO = sycl::memory_order::relaxed;
    // T expected = neutral<Type, AccT>();
...
void run_benchmark(size_t VECTOR_SIZE, size_t WORK_GROUP_SIZE, size_t ITEMS_PER_WORK_ITEM, queue& q) {
    ...
    // Initialize array
    q.parallel_for(VECTOR_SIZE, [=](id<1> i){ 
        A[i] = 1; 
        if(i == VECTOR_SIZE-1){
            A[i] = 0;
        }
        ...
        q.submit([&](handler &h){
            ...
            [=](nd_item<1> item){
                ...
                for(size_t i = 0; i < ITEMS_PER_WORK_ITEM*WORK_GROUP_SIZE; i+=WORK_GROUP_SIZE){
                    ...
                    if(idx < VECTOR_SIZE){
                        thread_sum = std::min(A[idx], thread_sum);
                    }
                }
                ...
                if(local_id == 0){
                    ...
                    atomicUpdate<T>(atomicRes, reduced_value);
                }
            });
        });
